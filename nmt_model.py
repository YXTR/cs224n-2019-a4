...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        packed_output, (last_hidden, last_cell) = self.encoder(packed)
        (padded_output), lengths = nn.utils.rnn.pad_packed_sequence(packed_output)
        enc_hiddens = padded_output.permute(1, 0, 2)
